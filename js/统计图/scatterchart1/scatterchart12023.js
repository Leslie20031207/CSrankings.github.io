const scatterCtx12023 = document.getElementById('scatterChart12023').getContext('2d');

// 定义背景色和边框色数组，与洲的颜色相对应
const continentColors2023 = {
...
const continentBorderColors2023 = {
...
function getBackgroundColorByContinent(continent) {
    return continentColors2023[continent] || 'rgba(0,0,0,0)'; // 默认背景色为透明
}

// 函数根据大学所属地区设置边框颜色
function getBorderColorByContinent(continent) {
    return continentBorderColors2023[continent] || 'rgba(0,0,0,1)'; // 默认边框色为黑色
}

// 准备散点图数据
const scatterChartData2023 = qs2023.map((university) => ({
    x: university['师生比'],
...
// 按洲分组
const groupedData = {};
scatterChartData2023.forEach((point) => {
