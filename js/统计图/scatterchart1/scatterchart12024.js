const scatterCtx12024 = document.getElementById('scatterChart12024').getContext('2d');

// 定义背景色和边框色数组，与洲的颜色相对应
const continentColors2024 = {
...
const continentBorderColors2024 = {
...
function getBackgroundColorByContinent(continent) {
    return continentColors2024[continent] || 'rgba(0,0,0,0)'; // 默认背景色为透明
}

// 函数根据大学所属地区设置边框颜色
function getBorderColorByContinent(continent) {
    return continentBorderColors2024[continent] || 'rgba(0,0,0,1)'; // 默认边框色为黑色
}

// 准备散点图数据
const scatterChartData2024 = qs2024.map((university) => ({
    x: university['师生比'],
...
// 按洲分组
const groupedData = {};
scatterChartData2024.forEach((point) => {
